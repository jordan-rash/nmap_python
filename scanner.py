...
class Scanner():
	...
	def __init__(self, flags = '', network = '127.0.0.1', ports = '1-1024', infile = None, wtd = None, async = None, preone = None, pretwo = None, prethree = None, prefour = None):
		self.inPorts = ports
		...
		if preone:
			...
			self.printOption = 3
			self.flags = '-n -Pn -sV -O -T4 --reason'
		elif prefour:
			slef.printOption = 4
			self.flags = ''
		else:
...
if __name__ == "__main__":
	...
	g1.add_argument('-3', help="Aggressive scan.  Will take a very long time.\nOPTIONS = -n -Pn -sV -O -T4 --reason", dest="prethree", action="store_true")
	g1.add_argument('-4', help="Place holder for scan type 4 with NSE support.", dest="prefour", action="store_true")
